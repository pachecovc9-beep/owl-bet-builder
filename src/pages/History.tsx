...
import { ArrowLeft, Trash2, Edit, Calendar, TrendingUp, Target, Zap, Activity } from 'lucide-react';
...
const History = () => {
  ...
  const [selectedBulletin, setSelectedBulletin] = useState<BettingBulletin | null>(null);
  ...
  const loadBulletins = () => {
    ...
    setBulletins(savedBulletins.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()));
  };
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'simple': return <Target className="h-4 w-4" />;
      case 'multiple': return <TrendingUp className="h-4 w-4" />;
      case 'live-simple': return <Zap className="h-4 w-4" />;
      case 'live-multiple': return <Activity className="h-4 w-4" />;
  ...
  const paginatedBulletins = bulletins.slice(startIndex, startIndex + ITEMS_PER_PAGE);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-foreground">Histórico de Boletins</h1>
              <p className="text-muted-foreground">
                {bulletins.length} boletim{bulletins.length !== 1 ? 's' : ''} guardado{bulletins.length !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
        </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
              {paginatedBulletins.map((bulletin) => (
                <Card key={bulletin.id} className="betting-card group hover:scale-105 transition-transform duration-200">
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        {format(bulletin.createdAt, 'dd MMM yyyy, HH:mm', { locale: pt })}
                      </div>
                      <div className="text-sm">
                        <span className="text-muted-foreground">{bulletin.games.length} jogo{bulletin.games.length !== 1 ? 's' : ''}</span>
                        <span className="text-primary font-bold ml-2">@{bulletin.totalOdds.toFixed(2)}</span>
                      </div>
                      {bulletin.stake && bulletin.potentialReturn && (
                        <div className="text-xs text-muted-foreground">
                          €{bulletin.stake.toFixed(2)} → €{bulletin.potentialReturn.toFixed(2)}
                        </div>
                      )}
                    </div>
