...
export const saveBulletin = (bulletin: BettingBulletin): void => {
  try {
    ...
    const updatedBulletins = [
      bulletin,
      ...existingBulletins.filter((b) => b.id !== bulletin.id),
    ];
    localStorage.setItem(
      BULLETIN_STORAGE_KEY,
      JSON.stringify(updatedBulletins)
    );
  } catch (error) {
...
export const deleteBulletin = (bulletinId: string): void => {
  try {
    const existingBulletins = getBulletins();
    const updatedBulletins = existingBulletins.filter(
      (b) => b.id !== bulletinId
    );
    localStorage.setItem(
      BULLETIN_STORAGE_KEY,
...
export const updateBulletinGameStatus = (
  bulletinId: string,
  gameId: string,
  ...
  try {
    ...
    if (bulletinIndex !== -1) {
      ...
      if (gameIndex !== -1) {
        ...
        const allGamesComplete = bulletin.games.every(
          (g) => g.status !== "pending"
        );
        if (allGamesComplete) {
